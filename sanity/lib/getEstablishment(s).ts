...
export async function getEstablishment(
  ...
  return client.fetch(
    `*[_type == "establishment" && slug.current == '${location}']{name, "displayImage": { "url": displayImage.asset->url, "alt": displayImage.alt}, about, continent, country, place, category, "pictures": pictures[]{ "url": picture.asset->url, "alt": picture.alt }, rating, priceLevel, address, contact, coordinates, reviews[]{_id, name, title, description, rating, "pictures": pictures[]{ "url": picture.asset->url, "alt": picture.alt }, date}, faqs[]{_id, question, answer}}`
  );
}
